...
public class OpenIdConnectProviderResource extends AwsResource implements Copyable<OpenIdConnectProviderCustom> {
    ...
    public void create(GyroUI ui, State state) throws Exception {
        ...
        if (getAutogenThumbprint()) {
            setAutoGeneratedThumbprint(getThumbprint(
                getUrl().replace("https://", "").split("/")[0],
                getAutogenThumbprintCertificatePort()));
    ...
    private String getThumbprint(String url, Integer port) {
        try {
            ...
            Certificate root = peerCertificates[peerCertificates.length - 1];
            return DigestUtils.sha1Hex(root.getEncoded());
    ...
    public List<ValidationError> validate(Set<String> configuredFields) {
        ...
        if (!getAutogenThumbprint() && getThumbPrints().isEmpty()) {
            errors.add(new ValidationError(
                this,
                null,
                "Either 'autogen-thumbprint' needs to be set to 'true' or 'thumb-prints' needs to be set."));
        }
