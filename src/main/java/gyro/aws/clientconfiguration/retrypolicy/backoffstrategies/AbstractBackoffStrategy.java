...
public abstract class AbstractBackoffStrategy implements BackoffStrategyInterface, ClientConfigurationInterface {

    private FixedDelay fixedDelay;
    private FullJitter fullJitter;
    private EqualJitter equalJitter;

    abstract String getBackoffStrategyType();
    ...
    public void validate() {
        ...
        if (count > 1) {
            throw new ClientConfigurationException(
                null,
                String.format(
                    "Only one of 'fixed-delay', 'full-jitter' or 'equal-jitter' is allowed for '%s'.",
                    getBackoffStrategyType()));
        } else if (count == 0) {
            throw new ClientConfigurationException(
                null,
                String.format(
                    "One of 'fixed-delay', 'full-jitter' or 'equal-jitter' is required for '%s'.",
                    getBackoffStrategyType()));
        }
    }
